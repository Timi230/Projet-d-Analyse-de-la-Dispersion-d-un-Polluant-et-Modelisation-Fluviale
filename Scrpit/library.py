...
def parti32():
    ...
    def U0(N): # matrice de taille N^2 obtenue par concaténation des lignes de la matrice (uij) pour t=0 
        x     = np.linspace(0,a,N+1) # à changer car x et y appartiennet à ]0,a[ 
    ...
    def SolutionCN_2(N):
        ...
        def mat_A(N):
            A = -4 * np.eye(N**2)
            A += np.diagflat(1*np.ones(((N**2)-1,1)),-3) + np.diagflat(1*np.ones(((N**2)-1,1)),-1)  + np.diagflat(1*np.ones(((N**2)-1,1)),1) + np.diagflat(1*np.ones(((N**2)-1,1)),3)
            A = A * (1/h**2)
            return A
